...
export function generateMap(world: World, seed: number): any[] {
    RNG.init(seed);
    ...
    const forest = new Biome(BiomeType.FOREST, new Vector(200, 200), new Vector(15000, 15000), 0xf);

    map.push([1, BiomeType.FOREST, 2, 2, 150, 150, 0xf]);

    function inside_map(i: number, j: number) {
        return i >= 0 && j >= 0 && i < nh && j < nw;
    }

    function add_single_resource(i: number, j: number, type: string, subtype: number) {
        if(i < 0 || j < 0 || i >= nh || j >= nw) return;
    ...
    function add_river_line(i: number, j: number, size: number, di: number, dj: number, mem: any, w: number, h: number) {
        ...
        for (let k = 0; k < size; k++) {
            for (let _i = i - h1; _i < i + h2; _i++) {
                for (let _j = j - w1; _j < j + w2; _j++) {
                    if(add_single_resource(_i, _j, "wtb", 0) === 1) mem.push([_i, _j, 1]);
                }
            }
    ...
    function add_single_river(i: number, j: number, current: number) {
        if (!inside_map(i, j)) return;
        ...
        if (inside_map(i - 1, j - 1) && (code & (8 + 2)) === 8 + 2 && tiles[i - 1][j - 1] !== undefined && tiles[i - 1][j - 1]["wtb"] !== undefined) code += 1;
        if (inside_map(i - 1, j + 1) && (code & (16 + 2)) === 16 + 2 && tiles[i - 1][j + 1] !== undefined && tiles[i - 1][j + 1]["wtb"] !== undefined) code += 4;
        if (inside_map(i + 1, j - 1) && (code & (8 + 64)) === 8 + 64 && tiles[i + 1][j - 1] !== undefined && tiles[i + 1][j - 1]["wtb"] !== undefined) code += 32;
        if (inside_map(i + 1, j + 1) && (code & (16 + 64)) === 16 + 64 && tiles[i + 1][j + 1] !== undefined && tiles[i + 1][j + 1]["wtb"] !== undefined) code += 128;
    ...
    function add_river(mem: any[]) {
        ...
        for (var __j = _j - 4; __j < _j + 4; __j++) add_single_resource(i - 1, __j, "wtb", 0);
        ...
        for (var __j = _j - 1; __j < _j + 2; __j++) add_single_resource(_i, __j, "wtb", 0);
    }

    function add_resources(type: string, subtype: number, amount: number, subpart?: number) {
        let x = 2;
        let y = 2;
        let w = 150;
        let h = 150;
    ...
    for (let i = 0; i < 6; i++) add_resources("t", i, Math.floor(80 * size));
    for (let i = 0; i < 4; i++) add_resources("b", i, Math.floor(80 * size));
    for (let i = 0; i < 3; i++) add_resources("s", i, Math.floor(50 * size));

    let mem: any = [];
    for (let i = 0; i < 6; i++) add_river(mem);
    render_river(mem);

    add_resources("p", 0, Math.floor(28 * size));

    for (let i = 0; i < 3; i++) add_resources("g", i, Math.floor(7 * size));

    for (let i = 0; i < 3; i++) add_resources("d", i, Math.floor(2 * size));
    for (let i = 0; i < 3; i++) add_resources("a", i, Math.floor(1 * size));
    for (let i = 0; i < 3; i++) add_resources("m", i, Math.floor(1 * size));
