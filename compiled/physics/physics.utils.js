...
class PhysicsUtils {
    static rayDistObstacle(entity, obstacle) {
    ...
    static rayInterceptsCircle(entity, obstacle) {
        const r = PhysicsUtils.rayDistObstacle(entity, obstacle);
        // console.log(r);
        return r < obstacle.radius && r > 0;
    }
    static getColliders(world, entity, radius, size = 2) {
        ...
        for (const obstacle of tiles) {
            const dist = entity.realPosition.distance(obstacle.realPosition);
            if (obstacle.type === tile_type_1.TileType.SAND)
                continue;
    ...
    static getObstacles(world, position, radius, size = 2) {
        ...
        for (const obstacle of entities) {
            ...
            if (!obstacle.getComponent("OBSTACLE" /* ComponentType.OBSTACLE */) || dist > radius + obstacle.radius || obstacle.position === position)
                continue;
            response.push(obstacle);
        }
        for (const obstacle of tiles) {
    ...
    checkCollision(entity, angle, width, height) {
    ...
    static getClosestPointOnCircle(entity, obstacle, stop) {
